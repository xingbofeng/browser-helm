...
| `bh_vision_batch_capture_full_pages` | Batch Capture Full Page Screenshots | `vision/` | `debug`, `vision` | `medium` | `scope?`, `maxTabs?` | 批量截取当前窗口页面长图；Agent/Runtime 调用需要明确批量媒体任务意图，结果原始图片仍会在 snapshot detail 中脱敏。 |
| `bh_vision_collect_images` | Batch Collect Page Images | `vision/` | `debug`, `vision` | `medium` | `scope?`, `maxTabs?`, `maxImagesPerTab?`, `includeCssBackgrounds?` | 批量获取当前窗口页面图片 URL 清单；Agent/Runtime 调用需要明确批量媒体任务意图，不下载图片二进制。 |
