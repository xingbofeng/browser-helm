...
- 将 v1.1.3 的 release gates 平台化：coverage trend、security scan、artifact provenance、prompt injection/security eval 和 failure taxonomy 统一进入平台报告。
- 实现 prompt injection eval cases。
- 承接完整 eval 框架：v1.1.2 只做最小安全回归，v1.2 只做 trace replay seed，本版本实现系统化 benchmark、failure taxonomy、reporter 和 prompt injection/security eval。
...
- T4.1 实现完整 eval framework：case registry、fixture runner、scorer、failure taxonomy、reporter、baseline comparison 和 CI-friendly output。
...
- T14. 增加平台文档。
- T15. 实现 release/eval dashboard：展示 coverage trend、security scan 结果、artifact provenance、benchmark score 和 prompt injection/security eval 结果。
