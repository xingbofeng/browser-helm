...
本期在不放开完整动作体系的前提下，增加 iframe read/click/type prototype。它们只用于验证 Action Readiness、PolicyEngine、approval hook、`changedPage` 和 `requiresObserve` 的真实链路；`iframe_submit` 仍明确不做。
...
- 增加 `动作准备 / Act` run mode，并保持 Ask/Debug/Form/Act 中英文双语展示。
- 增加 `bh_iframe_read`、`bh_iframe_click`、`bh_iframe_type` 原型；click/type 成功后必须返回 `changedPage=true` 和 `requiresObserve=true`。
- 补齐所有 `src/tools/**/bh-*.ts` 工具的 TSDoc/JSDoc 头部注释，并以 `src/tools/README.md` 清单作为维护索引。
...
- T10. 定义 Act mode 工具可见性和双语文案。
- T11. 实现 iframe read/click/type 受控原型，不暴露 `iframe_submit`。
- T12. 建立工具文档注释金标准，补齐历史工具。

## 4. 不做什么

本 Issue 明确不处理：

- 不做完整 click/type/nav 实现；本期只保留 iframe read/click/type 受控原型。
- 不做自动填写。
...
本阶段的 readiness 工具不实际修改页面。iframe click/type prototype 只在 `动作准备 / Act` 模式下可见，执行前必须通过 readiness 和 policy；高风险动作必须先进入 approval。
...
明确不需要实现的设计范围：完整普通页面 click/type/nav 执行器、`iframe_submit`、submit-with-approval、完整 approval UI、workflow replay。
...
ref -> action readiness -> risk classifier -> PolicyEngine -> ApprovalManager hook -> trace/tool result。iframe 原型链路为 `frame_<id>:ref_<id>` -> `bh_iframe_read` / `bh_iframe_click` / `bh_iframe_type` -> content RPC target frame -> `changedPage/requiresObserve`。
...
tests/node/tools/
├── frame/iframe-tools.test.ts
└── tool-documentation.test.ts
...
AC10. `动作准备 / Act`、`询问 / Ask`、`调试 / Debug`、`表单 / Form` 在 schema 和 side panel 中保持双语一致。

AC11. iframe read/click/type 原型通过单元、DOM 和 extension E2E 验证；click/type 成功后要求重新观察，敏感输入只进入 mask preview。

AC12. 所有 `bh_` 工具都有 TSDoc/JSDoc 头部注释，`ToolSpec.title` 前保留中文维护注释，`src/tools/README.md` 清单不漏项且不含旧路径。
