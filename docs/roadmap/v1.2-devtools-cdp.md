# v1.2 — DevTools/CDP Deep Tools

## 1. 背景

v1.0 的 debug 能力主要来自页面脚本可见的 console/network 摘要，无法深入 request body、response body、performance、event listeners 和 CDP target。前端开发者需要更接近 DevTools 的诊断能力。
...
Figma: 不适用
截图: docs/design/v1.2-devtools-cdp/
文档: docs/roadmap/v1.2-devtools-cdp.md
其他: GPT 生成设计图
...
明确不需要实现的设计范围：full DevTools replacement、source editor、profiler flamegraph。
...
background/debugger-manager 封装 chrome.debugger；CDP tools 经 ToolRouter 调用；敏感 headers/cookies 默认 mask。
...
CDP attach 失败不影响 v1.0 debug tools。Response body unavailable 要返回明确 reason。
...
tests/integration/background-debugger/
...
AC5. 已有 v1.0 shallow debug tools 不受影响。
...
feature/<issue-number>-devtools-cdp
