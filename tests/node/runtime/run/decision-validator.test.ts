...
describe('validateModelDecision', () => {
  ...
  it('does not treat conditional branch tool mentions as unconditional required tools', () => {
    const decision: AgentDecision = {
      type: 'finish',
      message: '页面被 Cloudflare 拦截，已读取可见文本。'
    };

    const record = makeRecord([
      '如果页面不是 Cloudflare 或 Just a moment 拦截，第一步必须调用 bh_form_read_fields 查找搜索框。',
