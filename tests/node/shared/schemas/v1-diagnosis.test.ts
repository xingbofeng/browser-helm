...
describe('v1.0 diagnosis schemas', () => {
      ...
      hiddenTools: [
        {
          ...
          reason: 'CDP capability is reserved for v1.3'
        }
      ],
      limitations: ['Debugger/CDP is unavailable in v1.0']
    });
  ...
  it('accepts findings and DebugReport with evidence and limitations', () => {
    ...
    const report = debugReportSchema.parse({
      ...
      limitations: ['未读取 response body，v1.0 仅支持浅层诊断']
    });

    expect(report.findings[0]?.confidence).toBe('high');
    expect(report.limitations?.[0]).toContain('v1.0');
  });
  ...
  it('accepts page health summary without CDP data', () => {
    const parsed = pageHealthSummarySchema.parse({
      ...
      limitations: ['Response body is not available in v1.0']
    });

    expect(parsed.networkFailures[0]?.method).toBe('GET');
    expect(parsed.limitations?.[0]).toContain('v1.0');
  });
});
